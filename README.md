...
- **Yatirim fonlari (TEFAS)**: `tefas.gov.tr`'nin kendi Next.js sitesinin ic (resmi olmayan) JSON uc noktalari (`/api/funds/fonGetiriBazliBilgiGetir` fon listesi/arama icin, `/api/funds/fonFiyatBilgiGetir` fiyat ve gecmis icin). Fon fiyatlari (NAV) Turkiye'de gunde bir kez (is gunu sonunda) yayinlandigindan, bu kategori icin fiyatlar saatte bir kez yenilenir (diger kategoriler gibi her yenileme araliginda degil) — bu normaldir, hata degildir. **BES (Bireysel Emeklilik Sistemi) fonlari da desteklenir**: ayni uc nokta `fonTipi` parametresine gore iki ayri liste dondurur — `YAT` yatirim fonlari (~1060 adet), `EMK` ise emeklilik yatirim fonlari (~310 adet). Fon kodlari iki liste arasinda cakismadigi icin (dogrulandi: sifir cakisma) tek bir sembol isim alanini paylasirlar. Arama sonuclarinda emeklilik fonlarinin alt satiri `BES · ...` ile isaretlenir, cunku `fonTurAciklama` alani her iki liste icin de ayni sozlugu kullanir ("Altin Fonu", "Degisken Fon") ve isaretlenmeseler yatirim fonlarindan ayirt edilemezlerdi. Fon adlarinda "BES" gecmedigi ("EMEKLILIK YATIRIM FONU" yazdigi) halde arama kutusuna `BES` yazmak da bu fonlari listeler.
